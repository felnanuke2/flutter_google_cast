...
public class SwiftGoogleCastPlugin: NSObject, GCKLoggerDelegate, FlutterPlugin, UIApplicationDelegate, GoogleCastContextHostApi {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    private func setSharedInstanceWithOption(option: GCKCastOptions) throws -> Bool {
    ...
    func setSharedInstanceWithOptions(request: CastContextInitRequest) throws -> Bool {
        ...
        guard let discoveryCriteria = options.discoveryCriteria else {
            throw NSError(
                domain: "CAST_ERROR",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Missing discoveryCriteria"]
            )
        }
        ...
        case "initWithApplicationID":
            guard let appId = discoveryCriteria.applicationID else {
                throw NSError(
                    domain: "CAST_ERROR",
                    code: 2,
                    userInfo: [NSLocalizedDescriptionKey: "Missing discoveryCriteria.applicationID"]
                )
            }
        ...
        option.physicalVolumeButtonsWillControlDeviceVolume = options.physicalVolumeButtonsWillControlDeviceVolume
        option.suspendSessionsWhenBackgrounded = options.suspendSessionsWhenBackgrounded
        option.disableDiscoveryAutostart = options.disableDiscoveryAutostart
        option.disableAnalyticsLogging = options.disableAnalyticsLogging
        option.stopReceiverApplicationWhenEndingSession = options.stopReceiverApplicationWhenEndingSession
        option.startDiscoveryAfterFirstTapOnCastButton = options.startDiscoveryAfterFirstTapOnCastButton
        stopCastingOnAppTerminated = options.stopCastingOnAppTerminated

        return try setSharedInstanceWithOption(option: option)
    }
